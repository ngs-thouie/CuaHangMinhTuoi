...
const ChatWindowBox: React.FC<{
  ...
  return (
    <div className="w-full sm:w-[330px] h-[70vh] sm:h-[450px] max-h-[80vh] bg-white border-t border-x border-sand shadow-2xl sm:rounded-t-lg flex flex-col overflow-hidden animate-slide-up">
      {/* Header */}
...
export const AdminChatManager: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 inset-x-0 sm:inset-x-auto sm:right-28 flex flex-row-reverse sm:gap-4 z-[9999] items-end pointer-events-none justify-end sm:justify-start">
        {openChats.map(c => (
          <div key={c.user._id} className="pointer-events-auto w-full sm:w-auto shrink-0">
            <ChatWindowBox 
              targetUser={c.user} 
